...
const doRawAndFlatten = function doRawAndFlatten(query, txn, flattenFn) {
	return (txn || conn).raw(query).then(response =>
		flattenFn(response).reduce((acc, item) => acc.concat(item), [])
	);
};

const getTables = async function getTables(txn) {
	/*
...
const getColumns = function getColumns(table, txn) {
