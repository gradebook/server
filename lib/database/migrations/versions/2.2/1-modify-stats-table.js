...
exports.up = ({connection}) => {
	return connection.schema.alterTable(TABLE_NAME, table => {
		...
		table.integer('accessed').notNullable().defaultTo(0);
		log.info('Added statistics.accessed');
		table.integer('accessedLastWeek').notNullable().defaultTo(0);
		log.info('Added statistics.accessedLastWeek');
		table.integer('categoriesUsed').notNullable().defaultTo(0);
