...
module.exports.getState = database => {
	let state;

	debug('Getting database state:', database);
	return migrator.isDatabaseOK().then(() => {
		debug('Database is ready:', database);
		state = 1;
		return state;
	}).catch(error => {
		const {code} = error;
...
module.exports._runMigration = database => me.getState(database).then(state => {
	if (state === 2) {
