...
function validateRow(tableName, model, {method}) {
	...
	for (const column of columns) {
		...
		const modalValue = model.get(column);
		// String conversion using basic _.toString logic
		const valueAsString = toString(modalValue);

		const {nullable, type, maxLength, validations, oneOf, defaultTo} = table[column];

		if (modalValue === null && !nullable) {
			errors.push(`${tableName}.${column} cannot be null`);
			continue;
		}

		// Is the value empty, the column not nullable, and no default value provided?
		if (isEmpty(valueAsString) && nullable !== true && defaultTo === undefined) {
		...
		if (type === 'boolean') {
			...
			if (!isEmpty(valueAsString)) {
				model.set(column, Boolean(modalValue));
			}
		} else if (type === 'integer' && !validator.isInt(valueAsString)) {
			if (valueAsString === '' && validator.isInt(String(defaultTo))) {
				model.set(column, defaultTo);
			...
			if (valueAsString === '' && validator.isFloat(String(defaultTo))) {
				model.set(column, defaultTo);
			} else if (valueAsString === '' && nullable) {
				model.set(column, null);
			// FLOAT(8, 2)
		...
		if (modalValue == null || modalValue == undefined) { // eslint-disable-line eqeqeq, no-eq-null
			continue;
		}

		// Check length
		if (maxLength && !validator.isLength(valueAsString, 0, maxLength)) {
			errors.push(`${tableName}.${column} exceeds the maxLength of ${maxLength}`);
			continue;
		}

		if (Array.isArray(oneOf) && !oneOf.includes(modalValue)) {
