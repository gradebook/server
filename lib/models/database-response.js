...
const serialize = require('../services/serializers/database-response/transform');
...
module.exports = class AbstractDatabaseResponse {
	constructor(responseObject) {
		...
		// @ts-ignore
		this._originalObject = serialize(responseObject, this.table, true);
		/** @type {{[s: string]: string}} */
		this._diff = {};
	...
	_getChangeSet() {
		const {diff} = this;
		return serialize.serializeObject(this.transformToSnakeCase, diff, true);
	}
	...
	*/
	async commit(txn = false, db = null) {
		...
		return serialize.serializeObject(this.transformFromSnakeCase, response, true);
	}
};
