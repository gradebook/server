...
module.exports = class AbstractDatabaseResponse {
	...
	* @returns {object} unSnakeCased object
	*/
	_transformToPublicObject(payload) {
		for (const [key, value] of Object.entries(payload)) {
			const normalKey = this.transformFromSnakeCase(key);

			if (normalKey !== key) {
				delete payload[key];
				payload[normalKey] = value;
			}
		}
	...
	*/
	 ...
	 */
	set(property, value) {
		const key = this.transformToSnakeCase(property);
		// CASE: invalid key
	...
	/** @returns {object} */
	get json() {
		return this._transformToPublicObject({
	...
	/** @returns {object} */
	get diff() {
		return this._transformToPublicObject({...this._diff});
	}
	...
	*/
	async commit(txn = false, db = null) {
		...
		return this._transformToPublicObject(response);
	}
};
