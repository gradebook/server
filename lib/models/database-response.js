...
module.exports = function createClassBasedOnSchema(tableName) {
	...
	return class DatabaseResponse {
		...
		async commit(transaction = false) {
			...
			try {
				response = await (transaction || knex)(tableName).update(changedCols).where({id});
			} catch (error) {
				if (!(error.code === 'ER_LOCK_DEADLOCK' && !transaction)) {
					throw error;
				}
				...
				response = await knex(tableName).update(changedCols).where({id});
			}
