...
const COURSE_JOIN_CAT = {'courses.id': 'categories.course_id', 'courses.semester': knex.instance.raw('"2020U"')};
const COURSE_JOIN_GRADE = {'courses.id': 'grades.course_id', 'courses.semester': knex.instance.raw('"2020U"')};
const COURSE_JOIN_GRADES_NOT = {'courses.semester': knex.instance.raw('"2020U"'), 'courses.id': 'grades.course_id'};
...
module.exports = async function runStatistics(fullDate, db = null) {
	...
	const [
		...
		knex({db, table: 'users'}).count('id as totalUsers').whereNotIn('id', blackListedUsers).first(),
		knex({db, table: 'users'}).count('id as newUsers').whereNotIn('id', blackListedUsers).andWhere('created_at', '>=', requestedDate).first(),
		knex({db, table: 'users'}).count('id as newUsersLastWeek').whereNotIn('id', blackListedUsers).andWhere('created_at', '>=', weekAgoDate).first(),
		knex({db, table: 'users'}).count('id as accessed').whereNotIn('id', blackListedUsers).andWhere('updated_at', '>=', requestedDate).first(),
		knex({db, table: 'users'}).count('id as accessedLastWeek').whereNotIn('id', blackListedUsers).andWhere('updated_at', '>=', weekAgoDate).first(),
		knex({db, table: 'grades'}).count('grades.id as gradesNotInCategories').whereNotIn('grades.user_id', blackListedUsers).innerJoin('courses', COURSE_JOIN_GRADES_NOT).whereNull('grades.name').first(),
		knex({db, table: 'courses'}).count('courses.id as totalCourses').where('semester', '2020U').innerJoin('users', qb => {
			qb.on('courses.user_id', 'users.id')
				.andOnNotIn('users.id', blackListedUsers);
		}).first(),
		knex({db, table: 'categories'}).count('categories.id as totalCategories').innerJoin('users', qb => {
			qb.on('courses.user_id', 'users.id')
				.andOnNotIn('users.id', blackListedUsers);
		}).innerJoin('courses', COURSE_JOIN_CAT).first(),
		knex({db, table: 'grades'}).count('grades.id as totalGrades').whereNotIn('grades.user_id', blackListedUsers).innerJoin('courses', COURSE_JOIN_GRADE).first(),
