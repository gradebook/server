...
module.exports = async function runStatistics(fullDate, db = null) {
	...
	const [
		...
		knex({db, table: 'users'}).count('id as totalUsers').first(),
		knex({db, table: 'users'}).count('id as newUsers').andWhere('created_at', '>=', requestedDate).first(),
		knex({db, table: 'users'}).count('id as newUsersLastWeek').andWhere('created_at', '>=', weekAgoDate).first(),
		knex({db, table: 'users'}).count('id as accessed').andWhere('updated_at', '>=', requestedDate).first(),
		knex({db, table: 'users'}).count('id as accessedLastWeek').andWhere('updated_at', '>=', weekAgoDate).first(),
		...
		knex({db, table: 'courses'}).count('id as totalCourses').where('semester', '2020U').first(),
