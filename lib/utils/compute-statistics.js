...
const offset = require('@gradebook/time').tzOffset.getOffset('America/Chicago');
...
module.exports = async function runStatistics(fullDate, db = null) {
	const ignoredUsers = [...ignoredUserService.getIgnoreUserList(db)];
	...
	const SEMESTER = knex.instance.raw('?', semesterService.activeSemester);

	const hasIgnoredUsers = ignoredUsers.length > 0;
	...
	const [
		...
		knex({db, table: 'grades'})
			...
			.whereNotIn('grades.user_id', ignoredUsers)
			.innerJoin('courses', qb => {
				qb.on('courses.semester', SEMESTER).andOn('courses.id', 'grades.course_id');
			})
		...
		knex({db, table: 'courses'})
			...
			.innerJoin('users', qb => {
				...
				if (hasIgnoredUsers) {
					qb.andOnNotIn('users.id', ignoredUsers);
				}
			})
			.first(),
		...
		knex({db, table: 'grades'})
			...
			.whereNotIn('grades.user_id', ignoredUsers)
			.innerJoin('courses', qb => {
				qb.on('courses.id', 'grades.course_id').andOn('courses.semester', SEMESTER);
			})
			.first(),
	...
	try {
		...
		await knex({db, txn, table: 'actions'})
			.whereIn('type', ['user.deleted', 'course.deleted', 'category.deleted'])
			.delete();
