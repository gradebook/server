...
class ThemeService {
	...
	constructor() {
		...
		*/
		...
		this._themes.set('default', {primary: '#5E3B4D', hover: '#7e4f67', background: 'assets/generic.jpg'});
		...
		this._themes.set('rebel', {primary: '#14213d', hover: '#2e4c8d', name: 'Rebel'});
		...
		this._themes.set('gator', {primary: '#0021a5', hover: '#0032fb', name: 'Gator'});
		this._themes.set('colonial', {primary: '#002654', hover: '#0056be', name: 'Colonial'});
		this._themes.set('commodore', {primary: '#866d4b', hover: '#ad926e', name: 'Commodore'});
		this._themes.set('tiger', {primary: '#f56600', hover: '#ff8a38', name: 'Tiger'});
		this._themes.set('wolfpack', {primary: '#cc0000', hover: '#ff1919', name: 'Wolfpack'});
	...
	*/
	getThemeForHost(hostname) {
		...
		if (!this._themes.has(school)) {
			return Object.assign({}, this._themes.get('default'));
		}
	...
	getCompiledTheme(hostname) {
		...
		<style>
			:root {
				...
				--image: url('${theme.background}') !important;
			}
		</style>`;
