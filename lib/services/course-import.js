...
/**
 * @param {string} slug
 * @param {typeof shrink['safelyRead']} [shrinkReader]
 */
export async function importFromSlug(slug, shrinkReader = shrink.safelyRead) {
	const course = await shrinkReader(slug);
...
/** @type {CourseImporter} */
export async function importCourse(databaseOptions, user, importIdentifier) {
	const parser = shrink.isProbablySlug(importIdentifier) ? importFromSlug : importFromHash;
	const course = await parser(importIdentifier);

	if (!course) {
		return;
	}

	await api.course.create({...course, user}, databaseOptions.db, databaseOptions.txn);
}
