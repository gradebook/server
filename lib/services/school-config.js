...
class SchoolConfigService {
	...
	*/
	...
	getSchoolConfig(hostname) {
		...
		const config = this.getRawSchoolConfig(hostname);
	...
	/**
	 * @param {string} hostname the host of the requested theme
	 * @returns {SchoolConfig} School configuration options
	 */
	getRawSchoolConfig(hostname) {
		const [school] = hostname.split('.');
		return this._schoolConfigs.get(school) || this._schoolConfigs.get('default');
	}
