...
export {MAX_CREDITS_IN_COURSE};

const loadReplacements = () => {
	const MAX_DROPPED_GRADES = Number(settings.get('max_grades_per_category')) - 1;
	return {
		partialCourseMeta: {
...
const replacer = isProduction ? replace : replaceWithDebug;
 ...
 */
export async function loadSchema(relativePath, yourRequire, replacementKey = 'none') {
