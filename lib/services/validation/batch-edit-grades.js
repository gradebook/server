...
export default function validateJob(request, _) {
	...
	for (let i = 0; i < body.delete.length; ++i) {
		if (!isObjectIDValid(body.delete[i])) {
			throw new errors.ValidationError({
				context: 'Validate delete', message: `Unable to update category: "${i}" is not a valid grade to delete`,
			});
		}
	}
	...
	for (let i = 0; i < body.update.length; ++i) {
		...
		if (!isObjectIDValid(grade.id)) {
			throw new errors.ValidationError({message: `Unable to update category: "${i}" is not a valid grade to edit`});
		}

		if (permissionIds.includes(grade.id)) {
			throw new errors.ValidationError({message: `Unable to update category: cannot edit and delete grade ${i}`});
		}
