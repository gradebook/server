...
module.exports = [{
	...
	function: async function runStatistics() {
		...
		const [
			...
			knex({table: 'users'}).count('id as totalUsers').whereNot('isNew', true).first(),
			knex({table: 'users'}).count('id as newUsers').whereNot('isNew', true).andWhere('created_at', '>=', requestedDate).first(),
			knex({table: 'users'}).count('id as newUsersLastWeek').whereNot('isNew', true).andWhere('created_at', '>=', weekAgoDate).first(),
			knex({table: 'users'}).count('id as accessed').whereNot('isNew', true).andWhere('updated_at', '>=', requestedDate).first(),
			knex({table: 'users'}).count('id as accessedLastWeek').whereNot('isNew', true).andWhere('updated_at', '>=', weekAgoDate).first(),
			knex({table: 'grades'}).count('grades.id as gradesNotInCategories').innerJoin('courses', COURSE_JOIN_GRADES_NOT).whereNull('grades.name').first(),
			knex({table: 'courses'}).count('id as totalCourses').where('semester', '2020S').first(),
			knex({table: 'categories'}).count('categories.id as totalCategories').innerJoin('courses', COURSE_JOIN_CAT).first(),
			knex({table: 'grades'}).count('grades.id as totalGrades').innerJoin('courses', COURSE_JOIN_GRADE).first(),
			knex({table: 'actions'}).select('data').where('type', '=', 'course.deleted'),
			knex({table: 'actions'}).select('data').where('type', '=', 'category.deleted'),
			knex({table: 'actions'}).select('data').where('type', '=', 'user.deleted')
		]);
		...
		try {
			await knex({txn, table: 'statistics'}).insert({
				date: justDate,
				totalUsers,
				newUsers,
				accessed,
			...
			await knex({txn, table: 'actions'}).delete().whereIn('type', ['user.deleted', 'course.deleted', 'category.deleted']);
