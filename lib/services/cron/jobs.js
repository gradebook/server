...
module.exports = [{
	...
	function: async function runStatistics() {
		const fullDate = date().add(offset, 'hours').add(-1, 'days');
		const requestedDate = fullDate.format('YYYY-MM-DD HH:00:00');
		const weekAgoDate = fullDate.add(-6, 'days').format('YYYY-MM-DD HH:00:00');
		...
		let newUsersWeek = (await knex('users').count('id as count').where('created_at', '>=', weekAgoDate).first()).count;
		...
		console.log({justDate, totalUsers, newUsers, totalCourses, totalCategories, totalGrades, deletedCourses, deletedCategories, deletedGrades, newUsersWeek});
	}
}, {
