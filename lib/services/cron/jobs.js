...
const COURSE_JOIN_CAT = {'courses.id': 'categories.course_id', 'courses.semester': knex.raw('"2020S"')};
const COURSE_JOIN_GRADE = {'courses.id': 'grades.course_id', 'courses.semester': knex.raw('"2020S"')};
const COURSE_JOIN_GRADES_NOT = {'courses.semester': knex.raw('"2020S"'), 'courses.id': 'grades.course_id'};
...
module.exports = [{
	...
	function: async function runStatistics() {
		...
		const [
			...
			knex('courses').count('id as totalCourses').where('semester', '2020S').first(),
