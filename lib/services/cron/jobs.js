...
module.exports = [{
	...
	// schedule: `*/1 * * * *`,   // Use this line to run statistics every 5 minutes for debugging
	...
	function: async function runStatistics() {
		...
		for (const user of allDeletedUsers) {
			...
			const [localUser, localCourse, localCat, localGrade] = unParsed.split('|');
		...
		for (const course of allDeletedCourses) {
			...
			const [localCourse, localCat, localGrade] = unParsed.split('|');
			deletedCourses += Number(localCourse);
			deletedCategories += Number(localCat);
		}

		for (const category of allDeletedCategories) {
			const unParsed = category.data;
			const [localCat, localGrade] = unParsed.split('|');
			deletedCategories += Number(localCat);
		}
