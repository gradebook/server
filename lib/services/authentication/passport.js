...
async function handleProfile(_, __, profile, callback) {
	...
	let firstNameFallback = displayName.substr(0, displayName.indexOf(' '));
	let lastNameFallback = displayName.substr(displayName.indexOf(' ') + 1);
...
function addStrategy() {
	...
	const callbackURL = new URL('./authentication/callback', config.get('oauth.callback', 'http://localhost:3000/')).href;
