...
module.exports = {
	...
	async import(data, transaction, db = null) {
		...
		try {
			...
			// Create course
			const course = await baseCreate(data.course, txn, db);
			const categories = [];
			...
			for (const config of data.categories) {
				// Get the course id
				config.course_id = course.id; // eslint-disable-line camelcase
				config.dropped_grades = config.dropped; // eslint-disable-line camelcase
				delete config.dropped;

				if (config.dropped_grades === 0) {
					config.dropped_grades = null; // eslint-disable-line camelcase
				}
				...
				const createGrade = name => {
					...
					grade.set('user_id', data.user);
					grade.set('course_id', course.id);
					grade.set('category_id', categoryId);
					grade.set('name', name);
					return grade;
				};
	...
	async delete(id, userId, db = null) {
		...
		try {
			...
			if (!ignoredUserService.isUserIdIgnored(db, userId)) {
