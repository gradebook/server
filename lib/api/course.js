		...
		try {
			...
			for (const config of data.categories) {
				...
				if (config.numGrades === 1) {
					...
					createdCategory.grade.push(grade.json);
				...
				for (let i = 1; i <= config.numGrades; i++) {
					...
					createdCategory.push(grade.json);
					if (!gradeBatches[i - 1]) {
						gradeBatches[i - 1] = [];
					}
			...
			for (const batch of gradeBatches) {
				await Promise.all(batch.map(grade => grade.commit())); // eslint-disable-line no-await-in-loop
			}
