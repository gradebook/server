...
module.exports = {
	browse(dataType, filter) {
		...
		return (...args) => {
			...
			if (typeof filter === 'function') {
				const secondParams = filter(whereParams, args);
				if (secondParams && secondParams !== whereParams) {
					whereParams = secondParams;
				}
			}

			return knex(table).select('*').where(whereParams);
		};
	},
	...
	create(modelName) {
		...
		return async (data, transaction) => {
			...
			const response = await single.commit(transaction);
	...
	read(dataType) {
		...
		return async id => {
			const object = await knex(table).select('*').where({id}).first();
			if (!object) {
	...
	update(modelName, additionalValidations = []) {
		...
		return async (id, responseInstance, data = {}, transaction) => {
			...
			const response = await responseInstance.commit(transaction);
	...
	delete(dataType, key = 'id') {
		...
		return async (value, txn) => {
			const query = {[key]: value};
			const numDeleted = await (txn || knex)(table).where(query).delete();
