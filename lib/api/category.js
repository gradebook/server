...
module.exports = {
	browse: (options, db = null) => {
		...
		if (options.userID) {
			joinParams['courses.user_id'] = knex.instance.raw('?', [options.userID]);
		}
	...
	async read(options, db = null) {
		...
		const joinParams = {
			...
			'categories.id': knex.instance.raw('?', [id])
		};
		...
		const response = await knex({db, table: 'categories'}).select(cols).join('courses', joinParams).first();
	...
	async create(data, transaction = false, db = null) {
