...
module.exports = {
	me: (req, res) => {
		res.status(200).json(sanitize(Object.assign({}, req.user)));
	},
	async approve(req, res) {
		...
		try {
			...
			await api.category.create({course_id, user_id, name: 'HW', weight: 20, position: 100}, txn);
			await api.category.create({course_id, user_id, name: 'Exam 1', weight: 25, position: 200, grade: 90}, txn);
			await api.category.create({course_id, user_id, name: 'Exam 2', weight: 25, position: 300, grade: 88}, txn);
			await api.category.create({course_id, user_id, name: 'Final', weight: 30, position: 400}, txn);
