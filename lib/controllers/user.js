// @ts-check
const SEMESTER = '2020S'; // @TODO: Dynamically generate this
...
module.exports = {
	me: (req, res) => {
		...
		res.status(200).json(sanitize(Object.assign({theme}, req.user || req.session.userProfile)));
	},
	approveFromSession(req, _, next) {
	...
	async approve(req, res) {
		...
		try {
			...
			const user = await api.user.create(profile, txn, database);
			...
			const course = await api.course.create({user_id, name: 'DEMO 101', semester: SEMESTER}, txn, database);

			const course_id = course.id;

			await api.category.create(
				{course_id, user_id, name: 'HW', weight: 20, position: 100}, txn, database
			);

			await api.category.create(
				{course_id, user_id, name: 'Exam 1', weight: 25, position: 200, grade: 90}, txn, database
			);

			await api.category.create(
				{course_id, user_id, name: 'Exam 2', weight: 25, position: 300, grade: 88}, txn, database
			);

			await api.category.create(
				{course_id, user_id, name: 'Final', weight: 30, position: 400}, txn, database
			);
			/* eslint-enable camelcase */

			await onUserCreate(profile.gid, user.id, req._table || 'default');
			await txn.commit();
	...
	async delete(req, res) {
		...
		try {
			const result = await api.user.delete(req.user.id, false, req._table, txn);
