...
if (createDomain) {
	...
	onUserCreate = async function sendUserToGlobalDb(gid, id, table) {
		await fetchEnhanced(endpoint, {
			method: 'PUT',
			body: JSON.stringify({gid, id, school: getSchoolFromTable(table)}),
			headers,
		});
	};

	onUserDelete = async function removeUserFromGlobalDb(gid) {
		await fetchEnhanced(`${endpoint}/${gid}`, {
			method: 'DELETE',
			headers,
		});
	};
...
export const controller = {
	...
	async reportAnIssue(request, response) {
		...
		const proxyPayload = {
			...
			partner: schoolConfigService.isPartnered(request._table),
		...
		try {
			...
			if (proxyResponse.status !== 200 && proxyResponse.headers.get('content-type') !== 'application/json') {
				logging.error(JSON.stringify({
					...
					text: await proxyResponse.text().catch(() => '(unavailable)'),
				}));
	...
	async approve(request, response) {
		...
		try {
			const profile = {...request.session.userProfile};
	...
	async updateSetting(request, response) {
		...
		const updatedSettings = JSON.parse(user.get('settings'));
	...
	/**
	 * // Remember: Validation happens before this so we know the key and value are valid
	 * @param {Gradebook.Request<never, never, never, never, GpaBody, {key: string}>} request
	 * @param {Gradebook.ResponseWithContext} response
	 */
	async updateGpaSettings(request, response) {
		const user = new UserModel({...request.user});
		/** @type {typeof request['user']['settings']} */
		const updatedSettings = JSON.parse(user.get('settings'));
		const {gpaSemester, overallCredits, overallGpa} = request.body;

		// @TODO: Drop this when we remove backward compatibility with client
		updatedSettings.overallCredits = overallCredits;
		updatedSettings.overallGpa = overallGpa;

		updatedSettings.gpaSemester = gpaSemester;
		updatedSettings.gpa[gpaSemester] = [overallGpa, overallCredits];

		const serializedUpdatedSettings = JSON.stringify(updatedSettings);

		// The likeliness of a noop is low; we'll pay the stringification cost to reduce complexity
		if (serializedUpdatedSettings === user.get('settings')) {
			response.context = {statusCode: 204};
			return;
		}

		user.set('settings', serializedUpdatedSettings);
		await user.commit(null, request._table);

		response.context = {body: updatedSettings};
	},
	...
	*/
	async export(request, response) {
		...
		exportedData.user = {...request.user};
