...
module.exports = {
	 ...
	 */
	me(request, response) {
		const theme = themeService.getThemeForHost(request._domain);
		response.context = {
			statusCode: 200,
	...
	async updateSetting(request, response) {
		...
		if (request.user.settings[key] === value) {
			return response.status(200).end({[key]: value});
		}
		...
		response.status(200).set('content-type', 'application/json').end(settingsString);
	},
	/**
	 * @param {import('../../global.d').Request} request
	 * @param {import('../../global.d').ResponseWithContext} response
	...
	async delete(request, response) {
		...
		try {
			const result = await api.user.delete(request.user, request._table, txn);
