...
const {validSemestersArray} = require('../services/current-semester');
...
module.exports = {
	...
	*/
	async browse(request, response) {
		...
		for (const course of courses) {
			if (validSemesters.has(course.semester)) {
	...
	*/
	async browseSlim(request, response) {
		...
		// STEP 1: Get courses
		const courses = await api.course.browse({user, semesters: validSemestersArray}, request._table);
		if (!Array.isArray(courses)) {
			return response.status(500).json(courses);
		}
		...
		// STEP 2: Get categories
		const categories = await api.category.browse({user, semester: semesterService.validSemestersArray}, request._table);
