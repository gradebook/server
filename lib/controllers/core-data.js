...
export const WAITING_FOR_CLIENT_UPDATE = true;
...
export const controller = {
	...
	*/
	async browse(request, response) {
		...
		const {includeSemesters} = request.query;
		const {activeSemesters, primarySemester} = semesterService;
		...
		for (const course of courses) {
			if (activeSemesters.includes(course.semester)) {
		...
		response.status(200).json({
			categories,
			courses,
			...((WAITING_FOR_CLIENT_UPDATE || includeSemesters === 'true') && {activeSemesters, primarySemester}),
		});
	},
	/**
	* @param {Gradebook.Request} request
	...
	*/
	async browseSlim(request, response) {
		...
		const semestersWithCourses = await api.course.browseSemesters(user, request._table);

		// STEP 1: Get courses
		const courses = await api.course.browse({user, semesters: activeSemesters}, request._table);
		if (!Array.isArray(courses)) {
			return response.status(500).json(courses);
		}
