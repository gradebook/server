...
module.exports = {
	async browse(req, res) {
		...
		const categories = await api.category.browse(filters);
	...
	async create(req, res) {
		const CATEGORY_PROPERTIES = ['course_id', 'name', 'weight', 'position'];
		const GRADE_PROPERTIES = ['course_id', 'grade'];

		const categoryModel = new NewCategory();
		const gradeModel = new NewGrade();

		gradeModel.set('user_id', req.user.id);

		for (const property of CATEGORY_PROPERTIES) {
			if (property in req.body) {
				categoryModel.set(property, req.body[property]);
			}
		}

		for (const property of GRADE_PROPERTIES) {
			if (property in req.body) {
				gradeModel.set(property, req.body[property]);
			}
		}
		...
		const response = await api.category.create(categoryModel, gradeModel);
	...
	async read(req, res) {
		...
		const category = await api.category.read({id: categoryID, includeCourse: true});
		...
		if (includeGrades === 'true') {
			const grades = await api.grade.browse({category: categoryID});
	...
	async edit(req, res) {
		const response = await api.category.update(req.params.id, req.queriedData, req.body);
		res.status(200).json(response);
	},
	async delete(req, res) {
		const {id: userID} = req.user;
		const {id: categoryID} = req.params;
		const wasDeleted = await api.category.delete(categoryID, userID);
	...
	async expand(req, res) {
		...
		const allGrades = await api.category.expand(category, req.permissions.user);
