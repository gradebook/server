...
module.exports = {
	...
	create: async (req, res) => {
		const {semester, name, cut1, cut2, cut3, cut4, cut1Name, cut2Name, cut3Name, cut4Name} = req.body;
		const {id: userID} = req.user;

		if ((cut1 && isNaN(cut1)) || (cut2 && isNaN(cut2)) || (cut3 && isNaN(cut3)) || (cut4 && isNaN(cut4))) {
			throw new errors.ValidationError({message: 'all cuts must be a valid number'});
		}

		// eslint-disable-next-line camelcase
		const course = await api.course.create({user_id: userID, semester, cut1, cut2, cut3, cut4, cut1Name, cut2Name, cut3Name, cut4Name, name}, null, req._table);
