...
module.exports = {
	...
	async import(req, res) {
		...
		try {
			const {id: userID} = req.user;
			const {semester, name, cut1, cut2, cut3, cut4, cut1Name, cut2Name, cut3Name, cut4Name, credits} = req.body.course;

			const createCategories = req.body.categories;

			const returnedCourse = await api.course.create({user_id: userID, semester, cut1, cut2, cut3, cut4, cut1Name, cut2Name, cut3Name, cut4Name, name, credits}, txn, req._table);
			let statusCode = returnedCourse.error ? 500 : 201;
			let sanitizedCategories = [];

			for (category of createCategories) {
				category.course_id = returnedCourse.id;
				category.user_id = userID;

				if (category.numGrades > 1) {
					// Expanded
				} else {
					delete category.numGrades;
					category.grade = null;

					const returnedCategory = await api.category.create(category, txn, req._table);
					statusCode = returnedCategory.error ? 500 : statusCode;
					sanitizedCategories.push(sanitizeCategory(returnedCategory));
				}
			}

			await txn.commit();
			res.status(statusCode).json({course: sanitizeCourse(returnedCourse), categories: sanitizedCategories});
		} catch (error) {
			await txn.rollback();
