...
module.exports = {
	...
	async import(req, res) {
		...
		try {
			...
			// Check status code of each item, starting with course
			let statusCode = returnedImport.course.error ? 500 : 201;
			const sanitizedCategories = [];

			// If any categories errored, 500 the whole request
			for (const category of returnedImport.categories) {
				statusCode = category.error ? 500 : statusCode;
				sanitizedCategories.push(sanitizeCategory(category));
			}

			await txn.commit();
			res.status(statusCode).json({
				course: sanitizeCourse(returnedImport.course),
