...
export const MAX_UPLOAD_FILE_SIZE = 20_971_520; // 20 MB
 ...
 * @param {import('express').NextFunction} next
 */
export function rejectLargeContentType(request, response, next) {
	const contentLength = Number.parseInt(request.headers['content-length'] ?? String(MAX_UPLOAD_FILE_SIZE + 1), 10);
	if (Number.isNaN(contentLength) || contentLength > MAX_UPLOAD_FILE_SIZE) {
		response.status(400);
		response.json({error: 'Request is too large'});
		response.end();
		return;
	}
