...
module.exports.app = async (request, response) => {
	...
	response.send(
		template.replace('__META_USER_STATE__', userPayload).replace(/__ASSETS_FOLDER__/g, assetsFolder).replace(
			'</head>',
			`${lr}${schoolConfigService.getSchoolConfig(request._domain).head}\n</head>\n<link rel="manifest" href="../manifest-${schoolConfigService.getRawSchoolConfig(request._domain).theme.primary.slice(1)}.webmanifest">\n</head>`
		)
	);
