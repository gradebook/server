// @ts-check
import {dayjs, semester} from '@gradebook/time';
...
export const app = async (request, response) => {
	...
	const thisSid = `${sRandom}.${++sid}`;
	const semesters = {
		primarySemester: semester.data.primarySemester,
		activeSemesters: semester.data.activeSemesters,
	};
	...
	// Session Identifier
	additionalMetadata += `<meta name="x-sid" value="${thisSid}" />\n`;
	// Live Reload (usually only used in development mode)
	additionalMetadata += liveReload + '\n';
	// Semester data (active and primary)
	additionalMetadata += `<meta name="x-semesters" value="${encodeURI(JSON.stringify(semesters))}" />\n`;
	...
	response.setHeader('x-gb-sid', thisSid);
	response.send(template.replace('</head>', `${additionalMetadata}\n</head>`));

	if (user.isFullUser) {
		if (!ignoredUsers.isUserIdIgnored(request.user.id)) {
