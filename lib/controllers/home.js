...
module.exports.app = async (request, response) => {
	...
	const theme = schoolConfigService.getCompiledTheme(request._domain);
	...
	let configTags = `\t<meta name="site-name" value="${encodeURI(theme.metadata.name)}" />\n`;
	configTags += `\t<meta name="site-config" value="${schoolConfigService.getSchoolConfig(request._domain).config}" />\n`;
