...
module.exports = {
	browse: async (req, res) => {
		...
		const grades = await api.grade.browse(queryOptions);
		let status = 500;
	...
	create: async (req, res) => {
		...
		const newGrade = await api.grade.create({user_id: user, course_id: course, category_id: category, grade, name});

		const statusCode = newGrade.error ? 500 : 201;
		return res.status(statusCode).json(sanitizeGrade(newGrade));
	},
	read(req, res) {
		res.status(200).json(sanitizeGrade(req.queriedData));
	},
	...
	async edit(req, res) {
		...
		const response = await api.grade.update(id, gradeObject, updatedData);

		res.status(200).json(sanitizeGrade(response));
	},
	delete: async (req, res) => {
		const {objectId: id} = req.permissions;
		const result = await api.grade.delete(id);
		...
		try {
			...
			if (req.permissions.update.length > 0) {
				for (const toUpdate of req.permissions.update) {
					...
					const diff = await api.grade.update(id, model, toUpdate, txn);
					updated.push(sanitizeGrade(diff));
				}
			}

			if (req.permissions.delete.length > 0) {
				await api.grade.deleteMultiple(req.permissions.delete, txn);
			}
			...
			if (req.permissions.create.length > 0) {
				for (const toCreate of req.permissions.create) {
					...
					// eslint-disable-next-line no-await-in-loop
					const createdGrade = await api.grade.create(toCreate, txn);
