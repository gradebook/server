...
module.exports = {
	...
	*/
	async browse(request, response) {
		...
		response.status(status).json(grades);
	},
	/**
	* @param {import('../../global').Request} request
	* @param {import('../../global').ResponseWithContext} response
	*/
	async create(request, response) {
		...
		return response.status(statusCode).json(sanitizeGrade(newGrade));
	},
	/**
	* @param {import('../../global').Request} request
	* @param {import('../../global').ResponseWithContext} response
	*/
	read(request, response) {
		response.status(200).json(sanitizeGrade(request.queriedData.json));
	},
	...
	*/
	async edit(request, response) {
		...
		response.status(200).json(sanitizeGrade(apiResponse));
	},
	/**
	* @param {import('../../global').Request} request
	* @param {import('../../global').ResponseWithContext} response
	*/
	...
	async batchEdit(request, response) {
		...
		try {
			...
			await txn.commit();
			response.status(200).json({created, updated});
		} catch (error) {
			await txn.rollback();
