...
/**
	...
	*/
function updateUserSessionIfNeeded(request, response, next) {
	...
	const updateSession = async () => {
		...
		const needsToUpdate = date(request.user.updated).diff(date(), 'day');

		// CASE: today - updated (in days) >= 0 (as in updated today or in the future)
		if (needsToUpdate >= 0) {
			return;
		}
...
export function useUserSessionUpdater(app) {
	app.use(updateUserSessionIfNeeded);
}
