...
module.exports.mount = app => {
	...
	app.get('/api/v0/me/export-data', pipeline({rateLimit: rateLimit('exportData'), validation: 'exportData', controller: user.export}));

	app.get('/api/v0/course/:id', pipeline({validation: 'readCourse', controller: course.read}));
	app.get('/api/v0/category/:id', pipeline({validation: 'readCategory', controller: category.read}));
	app.get('/api/v0/grade/:id', pipeline({validation: 'readGrade', controller: grade.read}));
	app.get('/api/v0/categories', pipeline({validation: 'browseCategory', controller: category.browse}));
	app.get('/api/v0/courses', pipeline({validation: 'browseCourse', controller: course.browse}));
	app.get('/api/v0/grades', pipeline({validation: 'browseGrade', controller: grade.browse}));
	app.get('/api/v0/core-data', wrap(coreData.browse));

	app.use(bodyParser.json({limit: '10kb'}));

	app.put('/api/v0/courses', pipeline({validation: 'createCourse', controller: course.create}));
	// Import course template
	app.put('/api/v0/courses/import', pipeline({
		rateLimit: rateLimit('importCourse'), validation: 'importCourse', controller: course.import
	}));
	app.post('/api/v0/course/:id', pipeline({validation: 'editCourse', controller: course.edit}));
	app.delete('/api/v0/course/:id', pipeline({validation: 'deleteCourse', controller: course.delete}));
	app.delete('/api/v0/semester/:semester', wrap(semester.delete)); // Delete courses by semester

	app.put('/api/v0/categories', pipeline({validation: 'createCategory', controller: category.create}));
	app.post('/api/v0/category/:id', pipeline({validation: 'editCategory', controller: category.edit}));
	app.post('/api/v0/category/:id/expand', pipeline({validation: 'expandCategory', controller: category.expand}));
	app.post('/api/v0/category/:id/contract', pipeline({validation: 'contractCategory', controller: category.contract}));
	app.delete('/api/v0/category/:id', pipeline({validation: 'deleteCategory', controller: category.delete}));

	app.put('/api/v0/grades', pipeline({validation: 'createGrade', controller: grade.create}));
	// Batch Edit Grades
	app.post('/api/v0/category/:id/batch', pipeline({
		rateLimit: rateLimit('batchEditGrades'),
		validation: 'batchEditGrades',
		controller: grade.batchEdit
	}));
	app.post('/api/v0/grade/:id', pipeline({validation: 'editGrade', controller: grade.edit}));
	app.delete('/api/v0/grade/:id', pipeline({validation: 'deleteGrade', controller: grade.delete}));

	app.post('/api/v0/me/settings', pipeline({validation: 'userSettings', controller: user.updateSetting}));
